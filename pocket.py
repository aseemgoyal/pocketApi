...
class Api(object):
    ...
    def _make_request(self, method, params):
        ...
        #print resp.read()
        decodedRes = resp.read().decode()
        var = json.loads(decodedRes)
#f1=open('./responseJson', 'w+')

#       f1.write(var)#print >> f1, var
        return var
    ...
    def new_items_list_from_json_dict(data):
        ...
        for item in data['list'].items():
            #            print(item)
            #print(item[1])
            #print(item[1].get('item_id'))
            #print(item[1].get('given_url'))
            
            temp = []
            if item[1].get('tags'):   # not empty
                for tag in item[1].get('tags'):
                    temp.append(tag)
                    temp.append(' ')
    ...
    def send(self, actions):
        '''Send allows you to make a change or batch several changes to a user
        list or Pocket data.
